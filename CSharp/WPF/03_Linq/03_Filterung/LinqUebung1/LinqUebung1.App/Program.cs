...
namespace LinqUebung1.App
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            Schueler s = db.Schuelers.Where(s => s.Id == 1003).First();

            Console.WriteLine($"{s.Name} {s.Vorname}");

            //var x = new
            //{
            //    result1.Vorname,
            //    Nachname = result1.Name,
            //    Klasse = result1.Klasse.ToLower(),
            //};
            //string name = x.Nachname;

            //x.Nachname = "";

            //IEnumerable<string> result2 = db.Schuelers.Select(s=>s.Name);

            //IEnumerable<string> result3 = db.Schuelers.Where(s => s.Name.Length >= 3).Select(s => s.Name);

            //IEnumerable<string> result4 = db.Schuelers.Select(s => s.Name).Where(s => s.Length >= 3);
