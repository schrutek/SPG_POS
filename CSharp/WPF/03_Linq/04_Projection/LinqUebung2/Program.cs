...
namespace LinqUebung2.App
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int uebung3 = db.Pruefungen.Select(p => p.Note).Max();

            var result4 = db.Schuelers.Select(s => new
            {
                s.Id,
                s.Name,
                Noten = s.Pruefungen.Select(p=>p.Note),
                s.Vorname,
            });
            ...
            var result4 = db.Schuelers
                ...
                new
                {
                    ...
                    Vorname = s.Vorname,
                    Name = s.Name,
                    Faecher = s.Pruefungen.Select(p => new { Fach = p.Fach }).Distinct()
                });
            ...
            var result6 = pruefers
                ...
                {
                    Pruefer = p,
                    BesteNote = db.Pruefungen.Where(pr => pr.Pruefer == p).Max(pr => pr.Note),
                    SchlechtesteNote = db.Pruefungen.Where(pr => pr.Pruefer == p).Min(pr => pr.Note),
                });
