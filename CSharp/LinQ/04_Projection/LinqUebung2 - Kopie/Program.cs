...
namespace LinqUebung2
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            //var p = new { Id = 1, Vorname = "asd", Name = "asd", Klasse = "sd", Geschlecht = "j" };
            ...
            // 4. Liefere eine Liste der Schüler (Name, Vorname) der 3BHIF mit ihren Prüfungsfächern. 
            ...
            var result4 = db.Schuelers
                ...
                .OrderBy(s => s.Klasse)
                .ThenBy(s => s.Name).Select(s => new
                {
                    Vorname = s.Vorname,
                    Name = s.Name,
                    Klasse = s.Klasse,
                    Faecher = s.Pruefungen
                        .Select(p => new
                        {
                            p.Fach
                        }).Distinct()
            ...
            var result5 = db.Schuelers
                ...
                new
                {
                    Name = s.Name,
                    Vorname = s.Vorname,
                    NegativePruefungen = s.Pruefungen.Where(s => s.Note == 5)
                    .Select(p => new
                    {
                        p.Datum,
                        p.Fach,
            ...
            IEnumerable<string> pruefers = db.Pruefungen.Select(p => p.Pruefer).Distinct();
            var result6 = pruefers
                .Select(pr => new
                {
                    Pruefer = pr,
                    BesteNote = db.Pruefungen.Where(p => p.Pruefer == pr).Max(p => p.Note),
                    SchlechtesteNote = db.Pruefungen.Where(p => p.Pruefer == pr).Min(p => p.Note),
                });
